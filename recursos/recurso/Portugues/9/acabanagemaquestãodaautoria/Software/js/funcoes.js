...
function mostraOpcoes(questaoId){
	...
	if (questoes[questaoId].pretexto) {
		html += questoes[questaoId].pretexto;
	}

	if (questoes[questaoId].dica) {
		html += questoes[questaoId].dica;
	}
    ...
    for (x=0;x<questoes[questaoId].itens.length;x++){
        ...
        if (item.foto) {
            html += '<div class="imagem"><img src="imagens/' + item.foto + '" /></div>';
        }
        ...
        case 'associacao':
            ...
            if (questoesSemResposta > 0){
                if (questao.respondida == 0 ){
                    ...
                    alert("Há ainda " + questoesSemResposta + " pergunta(s) sem resposta.");
                }
            } else {
                questao.respondida = 1;
            }
            ...
            for (var x = 1;x<=qtdeItens;x++){
             ...
             if (questoesSemResposta > 0){
                if (questao.respondida == 0 ){
                    ...
                    if(! confirm("Há ainda " + questoesSemResposta + " lacuna(s) sem resposta. Continuar ?")){
                        return false;
                    }
                }
            }
