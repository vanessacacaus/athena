package {
	...
	public class ConjuntoQuestoes {
		...
		private function defineQuestoesTeste() {
			...
			for (i=0; i < 5; i++) {
				q[i] = new Questao(tipo, perguntas[i], historia[i],respostas[i], dica[i], valorPontos[i], comentarios[i], respCorreta[i], diag[i]);
			}
		}
		...
		private function defineQuestoesVF() {
			...
			for (i=0; i < 5; i++) {
				q[i] = new Questao(tipo, perguntas[i], historia[i], respostas[i], dica[i], valorPontos[i], comentarios,respCorreta[i],diag[i]);
			}
		}
